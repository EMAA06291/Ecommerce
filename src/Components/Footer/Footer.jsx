...
export default function Footer() {
  return (
   ...
   <header >
        <h2 className="text-sm font-semibold text-slate-500-800 "> Get the fresh cart App </h2>
        <p className="text-slate-500-400">
          we will send you a link, open it on your phone to download the app
        </p>
      </header>
      <div className="flex gap-2 xsm:flex-col sm:flex-col md:flex-row">
        <input type="email" name="emaii" placeholder="enter your email"  className=" grow form-control"/>
        <button className="btn ms-2 uppercase font-semibold bg-primary-80 hover:bg-primary-400 text-white">share app link</button>
      </div>
      <div className=" ">
        <div className="flex justify-between items-center py-4 border-y-2 border-slate-300 border-opacity-50 ">
          <div className="payment-partners flex gap-3 items-center  xsm:flex-col md:flex-row">
            ...
            <img className="w-16" src={amazonPayLogo} alt="amamzon pay logo" />
            <img className="w-16" src={amircanExpressLogo} alt="american express logo" />
          ...
          <div className="download flex gap-3 items-center  xsm:flex-col md:flex-row">
            ...
            <img className="w-28" src={googlePlayLogo} alt="google play logo" />
            <img className="w-[107px]" src={appStoreLogo} alt="app Store Logo" />
          </div>
        </div>
      </div>
